...
async function example1_simpleAsk() {
  ...
  try {
    const response = await client.jobs.ask(userId, 'What is the price of Bitcoin?');
...
async function example5_batchQuestions() {
  ...
  const questions = ['What is Bitcoin?', 'What is Ethereum?', 'What is DeFi?'];
  ...
  try {
    ...
    const resultPromises = jobs.map((job) => client.jobs.poll(job.jobId, { interval: 1000 }));
...
async function example6_listAndManage() {
  ...
  try {
    ...
    console.log(
      `Found ${completedJobs.jobs.length} completed jobs (of ${completedJobs.total} total)`
    );
